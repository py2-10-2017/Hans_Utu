...
class UserManager(models.Manager):
    def validate_login(slef,post_data):
        errors = []
        ...
        if len(self.filter(email=post_data['email'])) > 0:
            ...
            if not bcrypt.checkpw(post_data['password']).encode(), user.password.encode()):
                errors.append('email/password do not match')
            else:
                error.append('email/password are incorrect')
    ...
    def validate_registration(self, post_data):
        ...
        if len(post_data['first_name']) < 2 or len(post_data['last_name']) < 2:
            errors.append("name fields must be at least 6 characters")
